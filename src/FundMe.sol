...
contract FundMe {
    ...
    receive() external payable {
        //if someone pays without deploying the contract. to keep track of them. used only when some amount of ether is sent without any calldata
        Fund();
    }

    fallback() external payable {
        //when ether is sent alongwith calldata is sent and nothing is found it comes back to fallback
        Fund();
    }
